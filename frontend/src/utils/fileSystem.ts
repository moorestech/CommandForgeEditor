...
/**
 * Loads sample-commands.yaml file for development fallback
...
export async function loadSampleCommandsYaml(): Promise<string> {
  try {
    ...
    if (import.meta.env.DEV) {
      ...
      // 開発環境以外の場合はTauriのファイル読み込みを使用
      throw new Error('Web mode is only available in development environment');
    }
  } catch (error) {
...
/**
 * Loads sample-skit.json file for development fallback
